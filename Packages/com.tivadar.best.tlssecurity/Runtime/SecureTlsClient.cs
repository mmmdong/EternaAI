...
namespace Best.TLSSecurity
{
    ...
    public class SecureTlsClient : AbstractTls13Client
    {
        ...
        public override void NotifyServerCertificate(TlsServerCertificate serverCertificate)
        {
            ...
            //    this._request?.Timing?.Add("Certificate validation", DateTime.UtcNow - startedAt);
            //}
        }
        ...
        List<X509Certificate> BuildCertificateChain(X509Certificate leafCertificate, List<X509Certificate> certs)
        {
            ...
            while (current != null)
            {
                ...
                if (parent == null)
                {
                    ...
                    {
                        for (int i = 0; i < matchedIntermediates.Count && parent == null; ++i)
                        {
                            var cert = matchedIntermediates[i] as X509Certificate;
                            if (cert != null && cert.IsValidNow)
                                parent = cert;
                        }
